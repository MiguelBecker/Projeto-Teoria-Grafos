...
\textbf{Fator de Pavimentação:}
\begin{itemize}
    \item Asfalto: $\times$ 1.0
    \item Paralelepípedo: $\times$ 1.3
    \item Escadaria: $\times$ 1.5
    \item Sem pavimentação: $\times$ 2.0
\end{itemize}

\textbf{Penalidades:}
\begin{itemize}
    \item Ponte/Viaduto: +0.5
    \item Semáforos: +0.3 cada
\end{itemize}

\section{4.3. Processamento de Dados}

\subsection{4.3.1. Etapa 1: Normalização dos Bairros}
...
\subsection{4.3.2. Etapa 2: Construção do Grafo}
...
\subsection{4.3.3. Etapa 3: Cálculo de Métricas}
...
\begin{itemize}
    \item Os pesos são estimativas baseadas em heurísticas, não medições reais de tempo ou distância
    \item O modelo não considera tráfego dinâmico ou horários de pico
    \item Algumas conexões viárias podem não estar mapeadas
    \item O sistema atual não suporta múltiplos modos de transporte
\end{itemize}
...
def calcular_peso(logradouro: str, observacao: str) -> float:
    ...
    if 'paralelepipedo' in obs_lower:
        fator_pav = 1.3
    elif 'escadaria' in obs_lower:
        fator_pav = 1.5
    elif 'sem pav' in obs_lower:
        fator_pav = 2.0
